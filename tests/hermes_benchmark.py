#!/usr/bin/env python3
"""ProgramDev Benchmark for Hermes Agent (single-agent baseline).

Tests Hermes Agent CLI against the same 30 ProgramDev tasks.
Compares single-agent (Hermes) vs multi-agent (ChatDev).

Usage:
    cd /tmp/mast-skills
    python3 -u tests/hermes_benchmark.py --subset 5 --easy-first
    python3 -u tests/hermes_benchmark.py --subset 30
    python3 -u tests/hermes_benchmark.py --subset 30 --config lean
"""
...
RESULTS_DIR = os.path.join(os.path.dirname(__file__), "results", "hermes")
WORKSPACE_BASE = "/tmp/hermes_benchmark"
...
def main():
    parser = argparse.ArgumentParser(
        description="ProgramDev benchmark for Hermes Agent")
    ...
    print()
    print("Comparison:")
    print(f"  Hermes ({args.config}):         this run")
    print("  Claude Code (baseline):       ~93%")
